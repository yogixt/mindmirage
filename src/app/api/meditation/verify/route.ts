...
const Body = z.object({
  ...
  residency: z.string().max(20).optional().default(""),
  mode: z.string().max(20).optional().default(""),
  slot: z.string().max(20).optional().default(""),
});
...
export async function POST(req: Request) {
  ...
  try {
    const { name, email, phone, residency, mode, slot } = parsed.data;
    await notify({
      _kind: "Booking",
      course: "Meditation · Level 01",
      sadhak: name || "Guest",
      email,
      phone,
      residency,
    ...
    console.error("[meditation/verify] notify failed", e);
  }
